...
## Phase 6C-D Diagnostic Addendum

Phase 6C-D added error analysis, threshold calibration, alternative scoring, and
triage on top of the real 20-pair full-agent run. The analysis does not change
repoDB labels, fabricate predictions, or tune on the test set.

Original full-mode confusion counts were 3 TP, 4 TN, 3 FP, 6 FN, and 4
partial/skipped rows. The low F1 was mainly a recall problem: most false
negatives came from completed full-agent outputs that used no PubMed, Open
Targets, or graph evidence for the pair, plus one conservative negative output.
False positives were also expected in this benchmark design because PubMed
co-mention and mechanistic evidence can exist for repoDB `negative_or_failed`
rows.

Threshold calibration used only dev rows, but the selected 20-pair run had only
3 evaluable completed dev rows, so calibration is explicitly exploratory. The
best F1 threshold for the original full confidence score was `0.55`; applied to
all 16 evaluable completed rows, it gave accuracy `0.5`, precision
`0.5714285714285714`, recall `0.4444444444444444`, F1 `0.5`, and ROC-AUC
`0.46825396825396826`.

Alternative transparent scores were computed from existing extracted features.
The strongest exploratory score was `safety_penalized_score`, with threshold
`0.06473928278103082`, accuracy `0.65`, precision `0.6`, recall `0.9`, F1
`0.7200000000000001`, and ROC-AUC `0.72` over the 20 selected rows. This should
be interpreted as a scoring-design signal, not a trained model result.

Triage classification added three labels: `likely_supported`,
`insufficient_or_negative`, and `uncertain_mixed`. It covered 13/20 rows,
abstained on 7/20, and reached accuracy `0.6153846153846154` on covered cases.
This supports a more reliable operating mode: abstain when evidence is mixed
instead of forcing every drug-disease pair into a binary answer.
...
## Phase 6F Scaling Preparation Addendum

Phase 6F prepares the full-agent diagnostics for 50- and 100-pair scaling. The
reason is straightforward: the 20-pair run was enough to reveal failure modes,
but not enough to trust threshold calibration. A 50-pair run is the minimum next
step because Open Targets and PrimeKG currently cover 50 pairs. A 100-pair run
is preferable if PubMed, Open Targets, graph, and LLM runtime/cost allow it.

The scaled pair selector created deterministic, label-balanced cohorts:
...
Selection prefers PubMed, Open Targets, and graph availability, but it does not
use prior full-agent correctness. This avoids selecting only easy successes.

In the current environment, `PUBMED_EMAIL` and `GEMINI_API_KEY` were not
configured, so PubMed expansion and real scaled full-agent execution were not
run. The 50- and 100-pair output directories contain explicit `TODO_NOT_RUN`
artifacts. No scaled predictions, reports, claims, PMIDs, citations, or metrics
were fabricated.

This means the 20-pair `safety_penalized_score` improvement remains promising
but still exploratory. It has not yet been confirmed or weakened by a real 50-
or 100-pair full-agent run. The verifier effect also remains supported by the
20-pair run only: full unsupported-claim rate `0.0625` versus `1.0` for
`no_verifier`.

## Phase 6F-B Real 50-Pair Scaled Evaluation

Phase 6F-B ran the real 50-pair scaled evaluation. PubMed coverage was expanded
to 50 rows first: 2,149 evidence rows, summed pair unique PMIDs `1058`, and
37/50 evidence-available pairs. The selected 50-pair cohort remained balanced:
25 positive, 25 negative_or_failed, 10 dev, and 40 test rows.

Full mode completed 42/50 pairs, produced 8 `partial_success` rows, and had 0
hard failures. Original full-confidence metrics were accuracy `0.5`, precision
`0.5714285714285714`, recall `0.5`, F1 `0.5333333333333333`, and ROC-AUC
`0.5173611111111112`.

The `no_verifier` ablation completed 41/50 pairs with 9 `partial_success` rows.
Its unsupported-claim rate was `1.0`, while full mode was
`0.10897435897435898`. The verifier effect therefore held up at 50 pairs:
verification did not magically solve label prediction, but it continued to
reduce unsupported claims sharply.

The best alternative score was again `safety_penalized_score`, with threshold
`0.13312792223387354`, accuracy `0.66`, precision `0.625`, recall `0.8`, F1
`0.7017543859649122`, and ROC-AUC `0.6464`. This strengthens the 20-pair
finding that raw full-agent confidence is not the best prediction score.
However, the result is still not clinical validation and should be described as
evidence strengthened, not proven.

Triage covered 25/50 rows, abstained on 25/50 rows, and achieved covered-case
accuracy `0.64` with covered-case F1 `0.39999999999999997`. This is useful as a
reliability mechanism because it explicitly marks mixed-evidence cases as
uncertain instead of forcing every pair into a binary claim.
