...
- Phase 6C-D full-pipeline diagnostics: error analysis, threshold calibration,
  alternative scoring, triage classification, and threshold figure generation.
- Phase 6F scale-up preparation: deterministic 50/100-pair selection, arbitrary
  run-directory diagnostics support, scaling comparison tables/figures, and
  release/demo scaling summaries.
...
- `eval_results/full_pipeline_scaled_50/per_pair_results_full.csv`
- `eval_results/full_pipeline_scaled_50/per_pair_results_no_verifier.csv`
- `eval_results/full_pipeline_scaled_50/diagnostics/alternative_score_comparison.csv`
- `eval_results/full_pipeline_scaled_50/diagnostics/triage_output.csv`
...
Latest result after Phase 6F-A scaling infrastructure: `125 passed`. Phase 6F-B
tests should be rerun after the real 50-pair documentation updates.
...
- Scale the full-agent run beyond 20 pairs after report generation is more
  stable.
- Run PubMed expansion to 50/100 pairs after `PUBMED_EMAIL` is configured.
- Run the selected 50/100 full-agent cohorts after `GEMINI_API_KEY` is
  configured; current scaled outputs are explicit `TODO_NOT_RUN` artifacts.
- Phase 6F-B completed PubMed expansion to 50 pairs and ran real 50-pair
  `full` plus `no_verifier` full-agent evaluations. Optional
  `no_target_expansion` and `no_graph_features` scaled ablations remain
  explicit `TODO_NOT_RUN` artifacts because of runtime/cost.
