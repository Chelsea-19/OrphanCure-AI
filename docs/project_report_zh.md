...
## Phase 6C-D 补充：错误分析、阈值校准和三分类分诊

Phase 6C-D 在真实 20-pair full-agent run 的基础上做诊断分析。这里没有修改
repoDB label，没有伪造 prediction，也没有用 test set 调参。

原始 full mode 的混淆矩阵是：TP = 3，TN = 4，FP = 3，FN = 6，partial/skipped =
4。F1 低的主要原因是 recall 低：6 个 false negatives 多于 3 个 false positives。
其中 5 个 FN 在 full run 中没有实际使用 PubMed、Open Targets 或 graph evidence；
另 1 个是模型在 positive proxy label 上给出了保守 negative 输出。FP 也有解释价值：
有的 case 是 PubMed co-mention 很强但 structured/graph support 很弱，有的
`negative_or_failed` row 本身带有 negative/failure 文献信号，说明 repoDB negative
不是干净的生物学负例。

阈值校准只使用 dev rows，但当前 20-pair selection 里只有 3 个 completed dev rows
可以评估，所以所有 calibrated thresholds 都标记为 exploratory。原始 full confidence
score 的 best F1 threshold 是 `0.55`；应用到 16 个 completed evaluable rows 上，
accuracy = `0.5`，precision = `0.5714285714285714`，recall =
`0.4444444444444444`，F1 = `0.5`，ROC-AUC = `0.46825396825396826`。

新的 alternative scores 全部来自已有 extracted features，不调用 LLM，也不生成新证据。
当前 exploratory 表现最好的方法是 `safety_penalized_score`，threshold =
`0.06473928278103082`，accuracy = `0.65`，precision = `0.6`，recall = `0.9`，
F1 = `0.7200000000000001`，ROC-AUC = `0.72`。这个结果只能说明 scoring design
有潜力，不能当作训练过的模型性能。

三分类分诊增加了 `likely_supported`、`insufficient_or_negative` 和
`uncertain_mixed`。它覆盖 13/20 rows，abstain 7/20 rows，在 covered cases 上
accuracy = `0.6153846153846154`。这比强制所有 case 二分类更适合 research-support
场景：证据混合时宁可标记 uncertain，也不假装模型已经有确定结论。
...
## Phase 6F 补充：为什么要 scale 到 50/100

20-pair run 的价值是发现 failure modes 和 verifier effect，但它不够支持稳定的
threshold calibration。只有 3 个 completed dev rows 可以用于原始 full-confidence
校准，所以 Phase 6C-D 的 `safety_penalized_score` 改进仍然是 exploratory。

Phase 6F 的目标是把 full-agent diagnostics 扩展到 50 或 100 pairs。50 pairs 是下一步
最低要求，因为当前 Open Targets 和 PrimeKG feature 覆盖正好是 50 pairs；100 pairs
更好，因为它会给 calibration 更多 dev rows，也能更稳定地检验 triage 和 verifier
effect。

本阶段已经生成 deterministic selected pair lists：
...
选择逻辑优先考虑 PubMed、Open Targets 和 graph availability，同时保持 label balance
和 dev/test split。它不使用之前 20-pair run 的 correctness，所以不是只挑 easy cases。

当前 shell 没有配置 `PUBMED_EMAIL` 和 `GEMINI_API_KEY`，所以没有运行 live PubMed
expansion，也没有运行真实 50/100 full-agent reports。输出目录中创建的是
`TODO_NOT_RUN` artifacts，没有伪造 predictions、reports、PMIDs、claims、citations
或 metrics。

面试解释可以这样说：20-pair 结果提示 `safety_penalized_score` 可能比 raw LLM
confidence 更适合 repoDB proxy label prediction，但这还不够强。Phase 6F 已经把
scale-up protocol、pair selection、diagnostics 和 public demo 展示层准备好；真正的
结论要等配置 API keys 后跑完 50/100 real full-agent outputs 才能确认。如果 50-pair
结果继续支持它，可以说 evidence strengthened but still not clinical validation；如果
50-pair 结果变弱，就说明 20-pair improvement unstable，不能隐藏负面结果。

## Phase 6F-B 补充：真实 50-pair scaled run

Phase 6F-B 已经跑完真实 50-pair scaled evaluation。先把 PubMed coverage 扩展到
50 rows：PubMed feature rows = 50，evidence rows = 2149，summed pair unique PMIDs =
1058，37/50 pairs 有 PubMed evidence。重新生成后的 50-pair cohort 仍然保持平衡：
25 positive，25 negative_or_failed，10 dev，40 test。

Full mode 结果：42 completed，8 partial_success，0 failed。原始 full-confidence
metrics 是 accuracy = `0.5`，precision = `0.5714285714285714`，recall = `0.5`，
F1 = `0.5333333333333333`，ROC-AUC = `0.5173611111111112`。

`no_verifier` ablation 结果：41 completed，9 partial_success，0 failed。它的
unsupported claim rate = `1.0`，而 full mode 是 `0.10897435897435898`。这说明
verifier effect 在 50-pair run 里仍然成立：verifier 不保证 clinical truth，也不保证
label accuracy，但它显著降低 unsupported claims。

Alternative scoring 里表现最好的仍然是 `safety_penalized_score`。threshold =
`0.13312792223387354`，accuracy = `0.66`，precision = `0.625`，recall = `0.8`，
F1 = `0.7017543859649122`，ROC-AUC = `0.6464`。这加强了 20-pair 发现：raw LLM
confidence 不是最可靠的 repoDB proxy prediction score；透明 evidence-based score
更稳定。但这个结论仍然不是 clinical validation。

Triage 覆盖 25/50 rows，abstain 25/50 rows，covered-case accuracy = `0.64`，
covered-case F1 = `0.39999999999999997`。面试时可以这样解释：triage 的价值不是
追求覆盖全部 case，而是在证据混合时主动说 uncertain，避免过度声称。
